...
export default function Bets({ gameId = null }) {
  ...
  // );
  const fetchKey = !gameId
    ? `admin/allbets?page=${currentPage}&limit=${pageSize}&search=${betSearch}`
    : `admin/getallgamebet?gameid=${gameId}&page=${currentPage}&limit=${pageSize}&search=${betSearch}`;

  let { data, error, isLoading } = useSWR(fetchKey, fetchData, {
    refreshInterval: 10000,
    ...
    <>
      ...
      <Pagination
        ...
        onChange={(page, pageSize) => {
          ...
          mutate(fetchKey); // Refresh data on page change
        }}
        onShowSizeChange={(current, size) => {
          setPageSize(size);
          setCurrentPage(1);
          mutate(fetchKey); // Refresh data on page size change
        }}
      />
    </>
  );
